...
- _sign：URL请求参数校验的MD5编码，由URI参数(按字母顺序)加第三方合作访问密钥联合组成

  > 例如：接口有3个URI参数 \_cid=test, \_ts=1234567890123, sid=12345, 密钥是abc，那么签名就是 \_sign=md5hex(\_cid=test&\_ts=1234567890123&sid=12345abc)=29241431d02fb983e7bdaf1cf26704e4

**在POST请求方式下，以上三个URI入参依然是在URI地址后直接进行附加，无需加入到表单中提交。**

---

### 系统响应封装

系统中各个接口的响应结果会以以下格式进行封装。
```
{
  "status": "OK",
  "result": {
